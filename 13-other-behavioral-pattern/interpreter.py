...
def main():
    ...
    for t in tests:
        try:
            parsed_items = event.parseString(t)
        except:
            continue
        if len(parsed_items) == 2:
            cmd, dev = parsed_items
            ...
            cmd_str, dev_str, arg_str = [' '.join(t) for t in parsed_items]

            num_arg = 0
            try:
                num_arg = int(parsed_items[-1][0])
            except:
                print(f'excepted number but got: "{parsed_items[-1][0]}"')
